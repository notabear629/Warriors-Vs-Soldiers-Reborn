...
async def renamerole(ctx, *, roleName=None):
    ...
    if roleName != None and roleName.lower().replace('а', 'a').replace('е', 'e').replace('о', 'o').replace('у', 'y').replace('х', 'x').replace('с', 'c').replace(' ', '').replace('/', '').replace('\\', '').replace('_', '').replace('*', '').replace('#','').replace('_', '').replace('ο', 'o').replace('ν', 'v').replace('ո', 'n').replace('օ', 'o').replace('ց', 'g') in bannedRoles:
