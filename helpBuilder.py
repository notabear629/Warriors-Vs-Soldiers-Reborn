...
class helpBuilder:
    ...
    async def expoActionsInfoEmbed(currentTheme, loadedRoles):
        ...
        returnedEmbed.add_field(name = f'{Daz.emoji}`Chicken Out`', value = f'This is an option that can be used once per game exclusive to {Daz.emoji}{Daz.shortName}{Daz.emoji}. This seems like a total meme choice, and to be honest, it kinda is, but it\'s also potentially insanely useful. How it works is that if you rejected the proposal for the {currentTheme.expeditionName} and are in it and forced to go on it anyway, you can choose this option and force it to be cancelled and make the next player start a proposal. Very importantly, and perhaps the best perk of the role, the Coward Out option appears IF AND ONLY IF you actually chose to reject. It DOES NOT CARE if the votes got flipped and forced you to accept, you will still retain the ability to cancel.', inline=True) 
        returnedEmbed.add_field(name = f'{Mikasa.emoji}`Guard`', value= f'This is an option excluse to {Mikasa.emoji}{Mikasa.shortName}{Mikasa.emoji} that can be used every single round they are in. This allows them to choose one person inside the expedition to make them immune from being killed, their choices includes but is not limited to themselves. They may choose to protect themselves OR somebody else. Or nobody.', inline=True)
