...
class helpBuilder:
    ...
    async def specialActionsInfoEmbed(currentTheme, loadedRoles, prefix):
        ...
        returnedEmbed.add_field(name = f'{Porco.emoji}`{prefix}ungag`', value = f'This is a way for {Porco.shortName} to un-do the usage of their ability and ungag the target.', inline=True)
    ...
    async def statsInfoEmbed(currentTheme):
        ...
        returnedEmbed.add_field(name = f'🐐`Legacy Points`', value = 'Legacy Points are a metric that is used to track the leaderboard for who is the #1 player. They are calculated by a sum of up to 1000 points from 5 categories including WORP, Total Wins, a fake-ass ELO analogue I made, MVPs, and Badge Points', inline=True)
